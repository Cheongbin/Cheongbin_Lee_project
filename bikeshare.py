...
def station_stats(df):
    ...
    most_common_s_e_st = df[['Start Station', 'End Station']].mode().loc[0]
    print("The most commonly used start station and end station : {}, {}"
          .format(most_common_s_e_st[0], most_common_s_e_st[1]))
...
def user_stats_gender(df):
    """Displays statistics of analysis based on the gender."""
...
def display_data(df):
    ...
    for i in range(0, row_length, 5):
        ...
        yes = input('\nExamine particular userdata? Type \'yes\' or \'no\'\n>')
        if yes.lower() != 'yes':
            break

        # retrieve and convert data to json format
        # split each json row data
        r_D = df.iloc[i: i+5].to_json(orient='records', lines=True).split('\n')
        for row in r_D:
